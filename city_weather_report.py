...
weather_homepage = "http://www.weather.com.cn/forecast/"
...
def get_pages_list(soup):   # 返回的是所有城市及天气网址的列表，元组
    ...
    for region_each in soup.find_all(class_="maptabbox",style="z-index:1000;"):        
        for each in region_each.div.ul.find_all('li'): # 嵌套循环 .find_all()大法好
            #print(each.a['href'],":",each.string)     #each.string是城市名,href是超链接网址
            
            city_url_dict[each.string] = each.a['href']   #把'城市名':'网址' 存入词典中
            
            each_dict={}
            each_dict[each.string] = each.a['href']
